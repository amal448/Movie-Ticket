    ...
    justify-between px-6 md:px-16 lg:px-36 py-5">
      ...
      ${isOpen?'max-md:w-full':'max-md:w-0'}`}>
        ...
        <Link onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} to='/'>Releases</Link>
       {
        ...
        <Link onClick={()=>{scrollTo(0,0);setIsOpen(!isOpen)}} to='/favorite'>Favorites</Link>
       }
      </div>
      {/* //login and search */}
      <div className="flex items-center gap-8">
        ...
        hover:bg-primary-dull rounded-full"
        onClick={openSignIn}>
          Login
        </button>
          ):(
      ...
      <MenuIcon onClick={()=>setIsOpen(!isOpen)} className='max-md:lg-4 md:hidden 
