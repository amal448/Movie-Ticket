...
const AddShows = () => {
  ...
  const handleIconClick = () => {
    if (inputRef.current) {
      // Triggers the browser's native date-time picker
      inputRef.current.showPicker?.()
      // For older browsers fallback: just focus
  ...
  const handleSubmit = async () => {
    try {
      ...
      const showInput = Object.entries(dateTimeSelection) //for destructure and
      //  reshape to pass as required
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      {/* Show Price Input */}
      <div className="mt-8">
        <label htmlFor="" className="block text-sm font-medium mb-2">Show Price</label>
      ...
      <div className="mt-6">
        ...
        <div className="inline-flex justify-center items-center gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
          <div className="relative">
            ...
            <input
              ...
              type="datetime-local"
              value={dateTimeInput}
              onChange={(e) => setDateTimeInput(e.target.value)} //show current selected
              className="pl-3 pr-10 outline-none rounded-md"
            />
          </div>
